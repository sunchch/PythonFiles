...
from reg_utils import predict, initialize_parameters, predict_dec, plot_decision_boundary, update_parameters
from reg_utils import backward_propagation, forward_propagation, compute_loss, load_2D_dataset, relu, sigmoid
import sklearn
...
def backward_propagation_with_regularization(X, Y, cache, lambd):
    ...
    Returns:
        gradients -- A dictionary with gradients with respect to each parameters, activation and pre-activation variables
    """
    m = X.shape[1]
...
def forward_propagation_with_dropout(X, parameters, keep_prob=0.5):
    ...
    D1 = np.random.rand(A1.shape[0], A1.shape[1])  # initialize matrix D1
    D1 = (D1 < keep_prob)  # convert entries of D1 to 0 or 1 (using keep_prob as the threshold)
    A1 = A1 * D1  # shutdown some neurons of A1
...
def model(X, Y, learning_rate=0.3, num_iterations=30000, print_cost=True, lambd=0, keep_prob=1):
    ...
    for i in range(0, num_iterations):
        ...
        if lambd == 0 and keep_prob == 1:  # 无正则化情况
